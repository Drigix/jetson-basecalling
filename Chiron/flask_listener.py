...
@app.route('/chiron/', defaults={'input_path': None, 'batch_size': None}, methods=['GET'])
@app.route('/chiron/<path:input_path>/<int:batch_size>', methods=['GET'])
def run_script(input_path, batch_size):
    try:
        if input_path is None or batch_size is None:
            # Scripts with default parameters
            command = ['bash', 'run_basecalling.sh', "/jetson-basecalling/test_sample/small/", str(140)]
